...
fn build() -> Result<(YogToml, PathBuf), String> {
    ...
    if let Some(facade) = generate_lib_facade(&meta) {
...
/// Generate the `yog_exports` facade module: `include!`s the mod's real
/// `src/lib.rs` verbatim (so authors never see or edit this file) and adds
/// `pub mod yog_exports { pub mod <name> { pub use ::<name>::*; } ... }` —
/// one submodule per `[dependencies.exports]` entry, so mod code writes
/// `yog_exports::yog_pipes::register_pipe(...)` regardless of how many
/// other Yog mods it depends on (each already has its own unique Cargo
/// alias, so nothing here can collide). Returns `None` when there are no
/// mod dependencies — `[lib] path` then skips this file entirely.
fn generate_lib_facade(meta: &YogToml) -> Option<String> {
